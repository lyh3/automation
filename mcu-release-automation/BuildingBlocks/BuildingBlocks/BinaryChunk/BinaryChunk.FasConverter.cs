/// <summary>
/// Copy right : https://www.codeproject.com/Tips/447938/High-performance-Csharp-byte-array-to-hex-string-t
/// Lisence : The Microsoft Public License (Ms-PL)
/// 
///------ Usages -------
/// byte array to hex string with "0x" prefix 
///string myHexString = BinaryChunk.ToHexString(myByteArray, true);
///
/// byte array to hex string without "0x" prefix 
///string myHexString = BinaryChunk.ToHexString(myByteArray);
///
/// hex string to byte array
///byte[] myByteArray = BinaryChunk.FromHexString(myHexString); 
/// 
/// </summary>
...
{
    public unsafe sealed partial class BinaryChunk
    {
            ...
            fixed (byte* sourceRef = source)
            {
                ...
                fixed (char* resultRef = result)
                {
                    ...
                    while (*pair != 0)
                    {
                        ...
                        *pair++ = 3145778; //add whitespace prefix
                        *pair++ = hexRef[*s++];
