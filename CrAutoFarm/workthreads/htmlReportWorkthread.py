...
class HtmlReportWorkThread(WorkThread):
    ...
    def SetupState(self, currentState, nextState):
        ...
        or (currentState is not None and isinstance(currentState, ArchiveDataState)):#OpenHtmlFileState)):
            ...
            if currentState is not None:
                ...
                self._logger.info('{0} : completed.'.format(type(currentState).__name__.replace('_state', '')))
                self._logger.info('Next test : {0}'.format(type(nextState).__name__.replace('_state', '')))
    ...
    def _writeStatistics(self, state):
        ...
        if isinstance(self.TestResultsDictionary, dict):
            for r in RESULTS:
