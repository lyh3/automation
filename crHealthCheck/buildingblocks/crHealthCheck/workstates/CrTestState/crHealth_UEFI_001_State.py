                ...
                if readTable:
                    ...
                    if len(re.findall('Channel', line)) > 0:
                        ...
                        if re.match('\d+\s+DIMM', line.replace('|', '')):
                            socketTableDictionary[tableName].append(columns)
                        if line.replace('|', '').strip()[0:4] in DDR4dimmSearchPattern:#.startswith(DDR4dimmSearchPattern):
                            socketTableDictionary[tableName].append(columns)
                        if line.replace('|', '').strip()[0:4] in NVDimmSearchPattern:#.startswith(NVDimmSearchPattern):
                            socketTableDictionary[tableName].append(columns)
