...
class Ipmitool(object):
    ...
    supported_interfaces = ['lan', 'lanplus', 'serial-terminal', 'ms', 'linux']
    ...
    def rmcp_invoke(self, params):
        ...
        output, rc = self._run_ipmitool(cmd)
        if rc != 0:
            raise RuntimeError('Failed to call ipmi [{0}], return code = {1}, {2}'.format(params, rc, check_completion_code(rc)))
        return output, rc
    ...
    def send_and_receive_raw(self, target, lun, netfn, raw_bytes):
        ...
        elif self._interface_type in ['ms']:
            cmd = self._build_ipmitool_cmd_ms(target, netfn, raw_bytes)
        elif self._interface_type in ['linux']:
            cmd = self._build_ipmitool_cmd_linux(target, netfn, raw_bytes)
        else:
            raise RuntimeError('interface type %s not supported' %
        ...
        #data = array('B')
        if match_completion_code or rc != 0:
            raise RuntimeError( "Failed to call pyipmi, error was:{0}".format(output))
        elif match_timeout:
            raise TimeoutError()
        else:
            ...
            output_lines = [l for l in output_lines if not l.startswith('Close Session command failed')]
            output = ''.join(output_lines).replace('\r','').strip()
            return output, rc

    def send_and_receive_raw_decimal(self, target, lun, netfn, raw_bytes):
        data = array('i')
        output = self.send_and_receive_raw( target, lun, netfn, raw_bytes)
        if len(output):
    ...
    def _build_ipmitool_target(self, target):
        ...
        if hasattr(target, 'routing'):
            ...
            if len(target.routing) == 1:
                ...
                #cmd += (' -b 0x0{:x}'.format(target.routing[0].bridge_channel))
    ...
    def _build_ipmitool_cmd_ms(self, target, netfn, raw_bytes):
        if not hasattr(self, '_session'):
            raise RuntimeError('Session needs to be set')
        cmd = self.IPMITOOL_PATH
        cmd += (' -I %s' % self._interface_type)
        cmd += self._build_ipmitool_target(target)
        cmd += self._build_ipmitool_raw_data(0, netfn, raw_bytes)
        cmd += (' 2>&1')
        return cmd

    def _build_ipmitool_cmd_linux(self, target, netfn, raw_bytes):
        if not hasattr(self, '_session'):
            raise RuntimeError('Session needs to be set')
        cmd = self.IPMITOOL_PATH
